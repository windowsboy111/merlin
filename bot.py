...
embed=discord.Embed()
def bash_shell(message,trash,_globals,_locals):
    global shell
    msg = message.content
    try:
...
def load_py(message,shell,_globals,_locals):
    ...
    while t.is_alive():
        if stop:
            return
def load_sh(message,shell,_globals,_locals):
    global stop
    t = threading.Thread(target=bash_shell,args=[message,shell,_globals,_locals])
    t.start()
    while t.is_alive():
        if stop:
            return
...
async def on_message(message):
    ...
    if type(message.channel) != discord.DMChannel and message.channel.name == 'python':
        ...
        stop=False
        return
    if type(message.channel) != discord.DMChannel and message.channel.name == 'bash':
        p = threading.Thread(target=load_sh,args=[message,shell,_globals,_locals])
        p.start()
        p.join(5)
        if p.is_alive():
            stop = True
